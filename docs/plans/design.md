# Design: Plan Tab & Training Plan View Redesign
...
2. Training Plan View = full plan understanding, analytics, and editing authority.
...
2. Make Plan Tab primarily calendar/scheduling focused with clear upcoming obligations.
3. Move Manage and Edit Structure controls exclusively to Training Plan View.
4. Add gallery-style chart cards in Training Plan View, specifically for adherence and readiness.
5. Meet navigation targets:
...
6. Deliver a mobile-friendly, minimal, interactive experience with familiar training-app conventions.
...
2. Interaction 2: Submit/Save from focused editor.
...
2. Next Up Card (largest card)
   - Required next session only.
   - CTAs: `Start`, `View Details`.
3. Upcoming Obligations List (next 72h)
...
5. Weekly Snapshot Strip
   - Planned vs completed volume and missed count only.
6. Lightweight Health Indicators
...
1. Adherence
2. Readiness (replace "Capability" label where still present)
...
2. Land in TPV with focused editor open.
3. Tap `Save`.

### Insight deep dive

1. Open Training Plan View.
2. Tap `Adherence` or `Readiness` card.
3. Review modal + optional action.

## Acceptance Criteria

1. Plan Tab shows schedule and upcoming obligations without full plan-detail duplication.
2. Manage Plan and Edit Structure controls are not present on Plan Tab.
3. Training Plan View contains adherence and readiness gallery cards with tap-to-modal details.
4. Users can reach any primary screen in one tap from Plan Tab.
5. Users can complete common edit+submit flows in two interactions from Plan Tab entry.
6. Migrating users (TrainingPeaks/Strava familiarity cohort) report >= 4/5 familiarity for core tasks.

## Validation & Analytics

Track post-launch events:

- `plan_tab_viewed`
- `required_item_impression`
- `required_item_opened`
- `training_plan_viewed`
- `gallery_card_opened_modal` (type: adherence/readiness)
- `record_edit_started`
- `record_submitted`
- `nav_primary_tap`
...
- Time to next required workout from Plan Tab
- Edit-and-submit interaction count
- Modal action follow-through rate
- Backtrack rate between Plan Tab and TPV

## Rollout Notes

1. Move controls first (Manage/Edit Structure migration) to establish ownership.
2. Introduce Plan Tab content reduction and obligation-first layout.
3. Add adherence/readiness gallery cards + modals in TPV.
4. Instrument and monitor interaction-count and discoverability metrics.
