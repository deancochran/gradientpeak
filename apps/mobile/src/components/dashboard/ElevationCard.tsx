...
import { Mountain, TrendingUp, TrendingDown, Activity, Navigation } from 'lucide-react-native';
import { useElevationMetrics, useDistanceMetrics } from '@/lib/hooks/useLiveMetrics';
...
export const ElevationCard: React.FC<ElevationCardProps> = ({ service, screenWidth }) => {
      ...
      <View style={{ width: screenWidth }} className="flex-1 p-4">
        <Card className="flex-1">
          <CardContent className="p-6 flex-1 items-center justify-center">
            <Icon as={Mountain} size={48} className="text-muted-foreground/20 mb-4" />
            <Text className="text-muted-foreground text-center">No elevation data</Text>
  ...
  const hasElevationData = elevationMetrics.totalAscent > 0 || elevationMetrics.totalDescent > 0;
      ...
      <Card className="flex-1">
        <CardContent className="p-6">
          ...
          <View className="flex-row items-center justify-between mb-6">
            ...
            {hasCurrentElevation && (
              <View className="flex-row items-center">
                <Icon as={Navigation} size={16} className="text-green-600 mr-1" />
          ...
          <View className="items-center mb-8">
            <Text className="text-4xl font-bold text-green-600">
              {hasCurrentElevation ? formatElevation(elevationMetrics.current!) : '---'}
            </Text>
            <Text className="text-sm text-muted-foreground">current elevation</Text>
          </View>
          ...
          <View className="flex-row justify-around mb-6">
            <View className="items-center">
              <View className="flex-row items-center mb-1">
                <Icon as={TrendingUp} size={16} className="text-green-500 mr-1" />
              </View>
            ...
            <View className="items-center">
              <View className="flex-row items-center mb-1">
                <Icon as={TrendingDown} size={16} className="text-blue-500 mr-1" />
              </View>
            ...
            <View className="items-center">
              <Text className={`text-xl font-semibold ${getGradeColor(elevationMetrics.avgGrade)}`}>
                {elevationMetrics.avgGrade > 0 ? '+' : ''}{elevationMetrics.avgGrade.toFixed(1)}%
              </Text>
          ...
          <View className="mb-6">
            <View className="p-4 bg-muted/10 rounded-lg">
              <View className="flex-row items-center justify-between mb-2">
                ...
                <Text className={`text-lg font-semibold ${getGradeColor(elevationMetrics.avgGrade)}`}>
                  {elevationMetrics.avgGrade > 0 ? '+' : ''}{elevationMetrics.avgGrade.toFixed(1)}%
                </Text>
              </View>
                ...
                <View className="h-2 bg-muted rounded-full overflow-hidden">
                  <View
                    className={`h-full rounded-full ${
                      elevationMetrics.avgGrade > 0 ? 'bg-green-500' : 'bg-blue-500'
                    }`}
                    style={{
          ...
          <View className="gap-3">
            {hasDistance && elevationMetrics.elevationGainPerKm > 0 && (
              <View className="flex-row justify-between items-center p-3 bg-green-500/10 rounded-lg">
                <View className="flex-row items-center">
                  <Icon as={TrendingUp} size={16} className="text-green-500 mr-2" />
            ...
            <View className="flex-row justify-between items-center p-3 bg-muted/10 rounded-lg">
              <View className="flex-row items-center">
                <Icon as={Activity} size={16} className="text-muted-foreground mr-2" />
                <Text className="text-sm font-medium">Net Change</Text>
              </View>
              <Text className={`font-semibold ${
                elevationMetrics.totalAscent - elevationMetrics.totalDescent > 0 ?
                'text-green-600' : 'text-blue-600'
              }`}>
                {elevationMetrics.totalAscent - elevationMetrics.totalDescent > 0 ? '+' : ''}
                {Math.round(elevationMetrics.totalAscent - elevationMetrics.totalDescent)}m
              </Text>
            </View>
            ...
            {hasElevationData && (
              <View className="p-3 bg-muted/10 rounded-lg">
                <Text className="text-xs text-muted-foreground mb-3">Elevation Profile</Text>
            ...
            {/* VAM (if climbing) */}
            {elevationMetrics.totalAscent > 50 && distanceMetrics && distanceMetrics.movingTime > 0 && (
              <View className="p-3 bg-orange-500/10 rounded-lg">
                <View className="flex-row items-center justify-between">
                  <View>
                    <Text className="text-xs text-muted-foreground mb-1">VAM</Text>
                    <Text className="text-lg font-semibold text-orange-600">
                      {Math.round((elevationMetrics.totalAscent / distanceMetrics.movingTime) * 3600)}
                    </Text>
