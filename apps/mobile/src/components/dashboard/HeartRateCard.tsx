      ...
      <View style={{ width: screenWidth }} className="flex-1 p-4">
        <Card className="flex-1">
          <CardContent className="p-6">
            ...
            {/* Current HR - Large Display with Placeholder */}
            ...
            <View className="p-4 bg-muted/10 rounded-lg items-center">
              <Icon as={Heart} size={32} className="text-red-500/50 mb-2" />
              <Text className="text-sm font-medium text-center mb-1">
                Heart Rate Monitoring Ready
              </Text>
              <Text className="text-xs text-muted-foreground text-center">
                Connect a heart rate monitor or start recording to see live data
              </Text>
            </View>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-6">
          ...
          <View className="mb-6">
            ...
            <View className="gap-2">
              {Object.entries(hrMetrics.zones).map(
                ([zoneKey, timeSeconds], index) => {
                  const minutes = Math.floor(timeSeconds / 60);
                  const totalZoneTime = Object.values(hrMetrics.zones).reduce(
                    (sum, t) => sum + t,
                    0,
                  );
                  const percentage =
                  ...
                  return (
                    <View key={zoneKey} className="flex-row items-center">
                      <View
                        className={`w-4 h-4 rounded mr-3 ${getHRZoneColor(index)}`}
                      />
                      <Text className="text-xs font-medium w-16">
                        Z{index + 1}
                      </Text>
              ...
              <View className="flex-row items-center justify-between">
                <View>
                  ...
                  <Text className="text-lg font-semibold">
                    {!hasCurrentHR
                      ? "Unknown"
                      : hrMetrics.current! < 100
                        ? "Very Light"
                        : hrMetrics.current! < 120
                          ? "Light"
                          : hrMetrics.current! < 140
                            ? "Moderate"
                            : hrMetrics.current! < 160
                              ? "Hard"
                              : hrMetrics.current! < 180
                                ? "Very Hard"
                                : "Maximum"}
                  </Text>
                </View>
