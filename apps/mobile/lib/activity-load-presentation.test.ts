...
describe("activity load presentation", () => {
  ...
  it("adapts specific canonical HR diagnostics", () => {
    ...
    expect(
      getCommonLoadPresentation({
        ...
        eligibleDurationSeconds: 3_000,
        sourceTimeCoverage: 0.4,
      })?.diagnostic,
    ).toMatchObject({ coveragePercent: 40, summary: expect.stringContaining("40%") });
    expect(
      getCommonLoadPresentation({
        ...heartRateUnavailable,
        contributingDurationSeconds: null,
