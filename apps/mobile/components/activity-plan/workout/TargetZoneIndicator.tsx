...
const TargetZoneIndicator = memo<TargetZoneIndicatorProps>(function TargetZoneIndicator({
  target,
  current,
}) {
  ...
  return (
    <View className="relative">
      ...
      <View className="flex-row justify-between mt-1">
        <Text className="text-xs text-muted-foreground">{Math.round(minVal)}</Text>
        <Text className="text-xs text-muted-foreground">{Math.round(maxVal)}</Text>
      </View>
    </View>
  );
});
