...
export function EllipticalControlUI({ service, controlMode, hasPlan }: EllipticalControlUIProps) {
  ...
  const applyResistance = useCallback(async () => {
    if (controlMode === "auto") {
      Alert.alert("Auto Mode Active", "Switch to Manual mode to adjust elliptical settings.");
      return;
    }
  ...
  const applyCadence = useCallback(async () => {
    ...
    const success = await service.applyManualTrainerCadence(targetCadence);
  ...
  const currentPower = currentReadings.power ? Math.round(currentReadings.power) : null;
  ...
  return (
    <View className="gap-6">
        ...
        <View>
          ...
          <View className="flex-row items-center gap-3 mb-3">
            ...
            <View className="flex-1 items-center">
              ...
              <Text className="text-xs text-muted-foreground mt-1">steps/min</Text>
            </View>
      ...
      <View className="bg-muted/30 p-3 rounded-lg">
        <Text className="text-xs text-muted-foreground">
          <Text className="font-medium">Tip:</Text> Higher resistance increases power output at the
          same cadence. Target cadence helps maintain consistent stride rate.
        </Text>
      </View>
    </View>
  );
