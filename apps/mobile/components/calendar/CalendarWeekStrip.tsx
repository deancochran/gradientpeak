...
type WeekDayButtonProps = {
  dateKey: string;
  indicators?: CalendarWeekDayIndicators;
  isSelected: boolean;
  isToday: boolean;
  onSelectDate: (dateKey: string) => void;
};
...
export function CalendarWeekStrip({
  ...
  const selectedWeekStartKey = getWeekStartKey(selectedDateKey);
  const weekPages = useMemo(() => buildWeekPages(selectedWeekStartKey), [selectedWeekStartKey]);
  ...
  const renderWeek = useCallback(
    ({ item }: ListRenderItemInfo<WeekPage>) => (
      <View
        ...
        <View className="flex-row justify-between gap-2">
          {item.dayKeys.map((dateKey) => (
            <WeekDayButton
              key={dateKey}
              dateKey={dateKey}
              indicators={dayIndicators.get(dateKey)}
    ...
    [dayIndicators, onSelectDate, selectedDateKey, todayKey, width],
  );
