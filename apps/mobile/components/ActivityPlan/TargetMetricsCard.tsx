...
const TargetMetricsCard = memo<TargetMetricsCardProps>(function TargetMetricsCard({
  target,
  current,
}) {
  ...
  return (
    <View className={`p-3 rounded-lg ${metricInfo.bgColor}`}>
      <View className="flex-row items-center justify-between mb-2">
        ...
        {inRange !== null && (
          <View className={`w-2 h-2 rounded-full ${inRange ? "bg-green-500" : "bg-orange-500"}`} />
        )}
      </View>
      ...
      <View className="flex-row items-end justify-between">
        ...
        <View>
          ...
          <Text className={`text-xl font-semibold ${metricInfo.color}`}>{targetDisplay}</Text>
          <Text className="text-xs text-muted-foreground">{metricInfo.unit}</Text>
        </View>
  ...
  if (!targets || targets.length === 0) {
    return (
      <View className="p-4 bg-muted/10 rounded-lg">
        <Text className="text-center text-muted-foreground">No targets for this step</Text>
      </View>
    );
  }
