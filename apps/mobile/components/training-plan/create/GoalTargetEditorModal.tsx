...
export function GoalTargetEditorModal({
  ...
  const renderActivityCategorySelect = (prefix: string) => (
    <View className="gap-2">
      <Label nativeID={`editor-${prefix}-category`}>
        ...
        value={
          editingContext.target.activityCategory
            ? {
                ...
                label:
                  getActivityCategoryLabel(editingContext.target.activityCategory) ?? "Activity",
              }
            : undefined
        }
      ...
      >
        ...
        <SelectContent>
          {activityCategoryOptions.map((option) => (
            <SelectItem key={`${prefix}-${option.value}`} label={option.label} value={option.value}>
              {option.label}
            </SelectItem>
          ))}
        </SelectContent>
  ...
  return (
    <AppFormModal
      ...
      testID="goal-target-editor-modal"
      title="Edit Target"
    >
      <ScrollView className="flex-1" contentContainerClassName="gap-4 pb-10">
        ...
        {editingContext.target.targetType === "race_performance" ? (
          <>
            {renderActivityCategorySelect("race")}
            ...
            <BoundedNumberInput
              ...
              required
              presets={(
                raceDistancePresetsByCategory[editingContext.target.activityCategory ?? "run"] ??
        ...
        {editingContext.target.targetType === "pace_threshold" ? (
          <>
            {renderActivityCategorySelect("pace")}
        ...
        {editingContext.target.targetType === "power_threshold" ? (
          <>
            {renderActivityCategorySelect("power")}
