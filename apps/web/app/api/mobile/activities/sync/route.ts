...
import { createActivity, getActivityById, updateActivitySync } from '@repo/drizzle/queries'
...
async function handleSingleSync(request: NextRequest, user: { id: string; email: string }) {
  const { activityId, startedAt, liveMetrics, filePath } = await validateRequest(request, SyncActivitySchema)
  ...
  try {
    ...
    if (existingActivity) {
      ...
      const activityData = {
        ...
        completedAt: new Date(startedAt).getTime() + (liveMetrics.duration * 1000), // Calculate end time
...
async function handleBulkSync(request: NextRequest, user: { id: string; email: string }) {
  const { activities } = await validateRequest(request, BulkSyncSchema)

  console.log('Syncing bulk activities:', activities.length, 'for user:', user.id)
    ...
    try {
      ...
      if (existingActivity) {
        ...
        const newActivityData = {
          ...
          profileId: user.id,
          startedAt: new Date(startedAt),
          completedAt: new Date(new Date(startedAt).getTime() + (liveMetrics.duration * 1000)),
          syncStatus: 'synced' as const,
          cloudStoragePath: filePath,
          ...liveMetrics
        }
