...
import { getActivityCount } from "@/lib/drizzle/queries";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = recentActivities.reduce(
      (acc, activity) => {
    ...
    const avgDistance =
      activityCount > 0 ? stats.totalDistance / activityCount : 0;
    const avgDuration =
      activityCount > 0 ? stats.totalDuration / activityCount : 0;
    ...
    const weeklyStats = Array.from(
      { length: Math.ceil(parseInt(period) / 7) },
      (_, weekIndex) => {
        ...
        return {
          ...
          totalDistance: weekActivities.reduce(
            (sum, a) => sum + (a.distance || 0),
            0,
          ),
          totalDuration: weekActivities.reduce(
