...
import { getProfileById, updateProfile } from '@repo/drizzle/queries'
...
const UpdateZonesSchema = z.object({
  ...
  zoneCalculationMethod: z.enum(['percentage', 'lactate_threshold', 'custom']).optional(),
  customHeartRateZones: z.array(z.object({
  ...
  customPowerZones: z.array(z.object({
    zone: z.number().min(1).max(7),
    name: z.string(),
    minWatts: z.number(),
    maxWatts: z.number(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (profile.maxHeartRate && profile.restingHeartRate) {
      heartRateZones = calculateHrZones(profile.maxHeartRate, profile.restingHeartRate)
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updatedProfile.maxHeartRate && updatedProfile.restingHeartRate) {
      heartRateZones = calculateHrZones(updatedProfile.maxHeartRate, updatedProfile.restingHeartRate)
    }
...
export async function POST(request: NextRequest) {
  try {
    const user = getUserFromHeaders(request)
    const { type, maxHeartRate, restingHeartRate, ftpWatts } = await validateRequest(request, RecalculateZonesSchema)

    console.log('Recalculating training zones for user:', user.id, 'type:', type)
