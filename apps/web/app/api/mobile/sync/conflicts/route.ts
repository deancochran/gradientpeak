...
import { getActivityById, updateActivitySync } from "@/lib/drizzle/queries";
...
async function handleSingleConflictResolution(
  ...
  const { activityId, resolution, mergeData } = await validateRequest(
    request,
    ConflictResolutionSchema,
  );

  console.log(
    "Resolving conflict for activity:",
    activityId,
    "resolution:",
    resolution,
  );
  ...
  switch (resolution) {
    ...
    case "skip":
      ...
      const [skipped] = await db
        ...
        .where(
          and(eq(activities.id, activityId), eq(activities.profileId, user.id)),
        )
        .returning();
...
async function handleBulkConflictResolution(
  ...
  const { conflicts } = await validateRequest(
    request,
    BulkConflictResolutionSchema,
  );

  console.log(
    "Resolving bulk conflicts:",
    conflicts.length,
    "for user:",
    user.id,
  );
  ...
  for (const conflict of conflicts) {
    try {
      ...
      console.error(
        "Error resolving conflict for activity:",
        conflict.activityId,
        error,
      );
  ...
  console.log("Bulk conflict resolution completed:", {
    successCount,
    errorCount,
  });
