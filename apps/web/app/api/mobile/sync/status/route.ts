...
export async function GET(request: NextRequest) {
  try {
    ...
    const statusMap = syncStatusCounts.reduce((acc, item) => {
      acc[item.syncStatus] = item.count
      return acc
    }, {} as Record<string, number>)

    // Get activities that need syncing (failed or local_only)
    ...
    const syncHealth = {
      ...
      failed: failedCount,
      syncPercentage: totalActivities > 0 ? Math.round((syncedCount / totalActivities) * 100) : 100
    }
    ...
    console.log('Sync status retrieved successfully for user:', user.id, syncHealth)
