...
description: "Helps with calculations, schemas, and business logic in database-independent core package."
model: sonnet
color: yellow
---
...
export function calculateNormalizedPower(
  ...
  const averageFourthPower = fourthPowers.reduce((sum, val) => sum + val, 0) / fourthPowers.length;
 ...
 */
export const activityPlanStepSchema = z.object({
...
export const activityPlanSchema = z.object({
  ...
  steps: z.array(activityPlanStepSchema).min(1, 'Plan must have at least one step'),
  createdAt: z.date(),
  updatedAt: z.date(),
});
