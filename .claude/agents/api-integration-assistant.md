...
description: "Helps integrate third-party APIs (Strava, Wahoo, Garmin, etc.) with proper auth, error handling, and data sync."
model: sonnet
color: cyan
---
...
export class StravaClient {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      throw new APIError(`Strava API error: ${response.statusText}`, response.status);
    }
  ...
  async getActivity(id: number): Promise<StravaDetailedActivity> {
    const activity = await this.request<StravaDetailedActivity>(`/activities/${id}`);
  ...
  async getActivityStreams(id: number): Promise<StravaStreams> {
    const types = ['time', 'latlng', 'altitude', 'heartrate', 'watts', 'cadence'];
...
export function mapStravaActivity(
  ...
  return activitySchema.parse({
    ...
    endTime: new Date(stravaActivity.start_date_local).getTime() + stravaActivity.moving_time * 1000,
 ...
 */
function mapActivityType(stravaType: string): 'run' | 'bike' | 'swim' | 'other' {
...
export const integrationsRouter = router({
  syncStravaActivities: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Fetch activities
      const after = input.since ? Math.floor(input.since.getTime() / 1000) : undefined;
      const stravaActivities = await client.getActivities({ after, perPage: 100 });
      ...
      for (const stravaActivity of stravaActivities) {
        try {
          ...
          errors.push(`Failed to import activity ${stravaActivity.id}: ${error.message}`);
        }
      }
