...
1. **NO MANUAL MIGRATION FILES**: Never manually create `YYYYMMDD_description.sql` files.
2. **DECLARATIVE WORKFLOW**:
   - Modify the source of truth (e.g., `packages/supabase/schemas/init.sql`) or local database state.
   - Run `supabase db diff` to auto-generate the migration file.
3. **SYNC TYPES**: Always run `pnpm update-types` after applying migrations. This updates both Zod schemas and database types.

## Your Responsibilities

1. Generate Supabase migration SQL files via `db diff`
2. Update TypeScript types after schema changes (`pnpm update-types`)
...
- `packages/supabase/migrations/` - Migration SQL files (generated)
- `packages/supabase/schemas/init.sql` - Source of truth for schema
...
(Note: These files are generated by the CLI, not manually created)
...
Modify the declarative schema file (e.g., `packages/supabase/schemas/init.sql`) to reflect the desired state.

### 2. Generate Migration (db diff)

Run the following command to compare the local schema/definitions against the migration history and generate the new migration file.

**Naming Rule**: The migration name must be in **lowercase kebab-case** (e.g., `add-performance-metrics`, `update-user-profiles`).

```bash
# Generate migration from diff with specific name
supabase db diff --use-migra -f <kebab-case-name>
```
...
**CRITICAL**: Always run this command after migration changes. It handles both database types and Zod schemas.
