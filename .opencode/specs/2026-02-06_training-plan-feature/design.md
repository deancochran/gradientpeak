...
The model must be explicitly shaped by four input groups:
	...
	∙	Goal type, target date/end date, activity category/ies, advanced config (constraints, progression settings) define the Ideal Path and expected adaptation slope
	2.	User training load history
	∙	Completed activity load history (TSS/HR load proxies) drives Actual Path plus rolling fitness/fatigue state (CTL/ATL/TSB or equivalent category-specific signals)
	...
	∙	Must leverage latest usable activity efforts by category
...
Rollout Strategy
	1.	Feature-flagged phased rollout: internal → small cohort → wider cohort
	2.	Shadow evaluation mode before full exposure
	3.	Rollback triggers on safety, error, or latency regressions
