...
function inputVariants({ className, editable }: { className?: string; editable?: boolean } = {}) {
  return cn(
    ...
    "bg-background text-foreground dark:bg-input/30 min-h-12 flex-row items-center leading-5 shadow-sm shadow-black/5 placeholder:text-muted-foreground/50",
...
function Input({ accessibilityLabel, id, role, testId, ...props }: InputProps) {
  ...
  return (
    <RegistryInput
      {...nativeTestProps}
      {...props}
      className={inputVariants(props)}
      style={{ ...StyleSheet.flatten(props.style), minHeight: 48 }}
    />
  );
}
