...
describe('RecordingConfigResolver', () => {
  describe('Outdoor Activities', () => {
    ...
    it('outdoor run without GPS - validation error', () => {
      ...
      expect(config.capabilities.errors).toContain('GPS is required for outdoor activities. Please enable location services.')
    })
  ...
  describe('Indoor Trainer Activities', () => {
    it('indoor trainer without plan - no steps, no auto control', () => {
      const input: RecordingConfigInput = {
      ...
      expect(config.capabilities.shouldShowSteps).toBe(false)
      expect(config.capabilities.canAutoControlTrainer).toBe(false) // No plan = no auto control
      expect(config.capabilities.canTrackPower).toBe(true)
      expect(config.capabilities.primaryMetric).toBe('power')
      expect(config.capabilities.warnings).toContain('Auto trainer control requires a structured plan. Control will be manual.')
    })
    ...
    it('indoor trainer with structured plan - shows steps, power target, auto control', () => {
      ...
      expect(config.capabilities.shouldShowMap).toBe(false)
      expect(config.capabilities.shouldShowSteps).toBe(true)
      expect(config.capabilities.shouldShowPowerTarget).toBe(true)
      expect(config.capabilities.canAutoAdvanceSteps).toBe(true)
      expect(config.capabilities.canAutoControlTrainer).toBe(true)
      expect(config.capabilities.canTrackPower).toBe(true)
      expect(config.capabilities.canTrackHeartRate).toBe(true)
  ...
  describe('Planned vs Unplanned', () => {
    it('planned without structure - warns and acts as unplanned', () => {
      ...
      expect(config.capabilities.warnings).toContain('Selected plan has no structure. Recording as unplanned.')
    })
  })
  ...
  describe('Device Connectivity', () => {
    it('no sensors for continuous activity - info warning', () => {
      ...
      expect(config.capabilities.warnings).toContain('No sensors connected. Metrics will be limited.')
    })
  ...
  describe('Edge Cases', () => {
    ...
    it('handles FTMS trainer with auto control disabled', () => {
      ...
      expect(config.capabilities.canAutoControlTrainer).toBe(false)
      expect(config.capabilities.shouldShowPowerTarget).toBe(true) // Still show targets
    })
  })
})
