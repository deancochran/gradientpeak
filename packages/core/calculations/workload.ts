export type SparseHistoryStatus =
...
export function getSparseHistoryStatus(
  coverageDays: number,
): SparseHistoryStatus {
...
export function computeTrimp(input: ComputeTrimpInput): WorkloadEnvelope {
  ...
  if (hasHrQuality && durationSeconds > 0 && maxHr > restingHr && avgHr > 0) {
    ...
    const trimp =
      ...
      hrReserveRatio *
      0.64 *
      Math.exp(sexFactor * hrReserveRatio);
