...
export type CompositeWeightKey = keyof TrainingPlanCalibrationConfig["readiness_composite"];

export type CompositeWeightLocks = Record<CompositeWeightKey, boolean>;

const COMPOSITE_KEYS: CompositeWeightKey[] = [
...
export function normalizeCompositeWeights(input: {
  ...
  rounded[rounded.length - 1] = round6((rounded[rounded.length - 1] ?? 0) + residual);
...
export function rebalanceCompositeWeights(input: {
  ...
  activeKey: CompositeWeightKey;
  nextValue: number;
}): TrainingPlanCalibrationConfig["readiness_composite"] {
  const current = { ...input.weights };
  const next = clamp01(input.nextValue);

  const fixedKeys = COMPOSITE_KEYS.filter((key) => key === input.activeKey || input.locks[key]);
  ...
  const variableTotal = variableKeys.reduce((sum, key) => sum + clamp01(current[key]), 0);
