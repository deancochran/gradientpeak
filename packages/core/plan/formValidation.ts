...
const goalSchema = z
  .object({
    ...
    targetDate: z.string(),
    priority: z.number(),
    targets: z.array(goalTargetSchema).min(1, "At least one target is required"),
  })
...
export function getMinimumGoalGapDays(goals: { targetDate: string }[]): number | undefined {
  ...
  for (let index = 0; index < sortedGoalDates.length - 1; index += 1) {
    ...
    const gapDays = Math.floor((nextDateUtc.getTime() - currentDateUtc.getTime()) / DAY_MS);
