...
const fullProjectionBenchmark =
  process.env.RUN_FULL_PROJECTION_BENCHMARK === "1" ? it : it.skip;
...
describe("periodization benchmarks", () => {
  ...
  it("keeps reference generation under 10ms for a 365-day 3-goal plan", () => {
  ...
  fullProjectionBenchmark(
    "keeps full projection under 50ms for a 365-day 3-goal plan",
    () => {
      const averageMs = averageDurationMs(() => {
