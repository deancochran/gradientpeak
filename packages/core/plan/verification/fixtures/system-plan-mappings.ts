...
function collectStringField(node: unknown, fieldName: string): string[] {
  ...
  const directValue =
    typeof record[fieldName] === "string" ? [record[fieldName] as string] : [];

  return [
    ...directValue,
    ...Object.values(record).flatMap((value) =>
      collectStringField(value, fieldName),
    ),
  ];
}
...
function collectFieldValues(node: unknown, fieldName: string): unknown[] {
  ...
  return [
    ...directValue,
    ...Object.values(record).flatMap((value) =>
      collectFieldValues(value, fieldName),
    ),
  ];
}
...
const systemTemplateById = new Map(
...
function buildSystemPlanCatalogEntry(
  ...
  const linkedActivityTemplates = resolveLinkedActivityTemplates(
    plan.structure,
  );
  ...
  if (
    declaredCycleWeeks !== null &&
    declaredCycleWeeks !== materializedSessionWeeks
  ) {
    limitations.push(
...
const catalogById = new Map(
  SYSTEM_PLAN_FIXTURE_CATALOG.map((entry) => [entry.plan_id, entry]),
);
...
function requireCatalogEntry(planId: string): SystemPlanFixtureCatalogEntry {
  ...
  if (!entry) {
    throw new Error(
      `Verification fixture plan catalog missing entry ${planId}`,
    );
  }
    ...
    audit: buildAuditMetadata({
      ...
      auditFields:
        limitations.length > 0
          ? ["scenario_to_plan_semantic_fit", ...plan.audit.audit_fields]
          : [],
      limitations,
    }),
  };
...
export const SYSTEM_PLAN_MAPPING_FIXTURES_BY_SCENARIO =
  SYSTEM_PLAN_MAPPING_FIXTURES.reduce(
    (accumulator, fixture) => {
