...
export const athleteIntelligenceSnapshots = pgTable(
  ...
  (table) => [
    uniqueIndex("athlete_intelligence_snapshots_idx_key").on(table.idx),
    unique("athlete_intelligence_snapshots_id_profile_id_unique").on(table.id, table.profile_id),
    unique("athlete_intelligence_snapshots_profile_fingerprint_unique").on(
      table.profile_id,
      table.input_fingerprint,
    ),
    index("idx_athlete_intelligence_snapshots_profile_created_at").on(
      table.profile_id,
      table.created_at,
    ),
  ],
...
export const athleteIntelligencePredictions = pgTable(
  "athlete_intelligence_predictions",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    idx: serial("idx").notNull(),
    profile_id: uuid("profile_id")
    ...
    prediction_kind: text("prediction_kind").notNull(),
    scenario_key: text("scenario_key").notNull().default(""),
    schema_version: text("schema_version").notNull(),
    payload: jsonb("payload").notNull(),
    created_at: timestamp("created_at", { withTimezone: true, mode: "date" })
      .defaultNow()
  ...
  (table) => [
    uniqueIndex("athlete_intelligence_predictions_idx_key").on(table.idx),
    foreignKey({
      columns: [table.snapshot_id, table.profile_id],
      foreignColumns: [athleteIntelligenceSnapshots.id, athleteIntelligenceSnapshots.profile_id],
      name: "athlete_intelligence_predictions_snapshot_profile_fkey",
    }).onDelete("cascade"),
    unique("athlete_intelligence_predictions_idempotency_unique").on(
      table.snapshot_id,
      table.model_version,
      table.prediction_kind,
      table.scenario_key,
    ),
    index("idx_athlete_intelligence_predictions_profile_created_at").on(
      table.profile_id,
      table.created_at,
    ),
  ],
...
export const athleteIntelligenceRecomputeJobs = pgTable(
  "athlete_intelligence_recompute_jobs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    idx: serial("idx").notNull(),
    profile_id: uuid("profile_id")
      .notNull()
      .references(() => profiles.id, { onDelete: "cascade" }),
    target_input_fingerprint: text("target_input_fingerprint").notNull(),
    reason_codes: jsonb("reason_codes").notNull().default(sql`'[]'::jsonb`),
    status: text("status").notNull().default("queued"),
    attempt_count: integer("attempt_count").notNull().default(0),
    max_attempts: integer("max_attempts").notNull().default(8),
    run_at: timestamp("run_at", { withTimezone: true, mode: "date" }).notNull().defaultNow(),
  ...
  (table) => [
    uniqueIndex("athlete_intelligence_recompute_jobs_idx_key").on(table.idx),
    uniqueIndex("athlete_intelligence_recompute_jobs_active_profile_unique")
      .on(table.profile_id)
      .where(sql`${table.status} in ('queued', 'running')`),
    index("idx_athlete_intelligence_recompute_jobs_claim").on(
      table.status,
      table.run_at,
      table.lock_expires_at,
    ),
    index("idx_athlete_intelligence_recompute_jobs_profile_status").on(
      table.profile_id,
      table.status,
    ),
    check(
      "athlete_intelligence_recompute_jobs_status_check",
      sql`${table.status} in ('queued', 'running', 'completed', 'failed')`,
    ),
    check(
      "athlete_intelligence_recompute_jobs_attempt_count_check",
      sql`${table.attempt_count} >= 0 and ${table.attempt_count} <= ${table.max_attempts}`,
    ),
  ],
);
