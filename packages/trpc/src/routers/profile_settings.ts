...
function coerceLegacyProfileSettings(settings: unknown) {
  ...
  const constraints = legacyShape.constraints.safeParse(
    rawSettings.constraints,
  );
  const behaviorControls = legacyShape.behavior_controls_v1.safeParse(
    rawSettings.behavior_controls_v1,
  );
  const recentInfluence = legacyShape.recent_influence.safeParse(
    rawSettings.recent_influence,
  );
  ...
  return athleteTrainingSettingsSchema.parse({
    ...
    dose_limits: {
      ...
      max_single_session_duration_minutes:
        constraints.data.max_single_session_duration_minutes,
    },
    training_style: {
    ...
    adaptation_preferences: {
      ...
      recency_adaptation_preference:
        (recentInfluence.data.influence_score + 1) / 2,
    },
